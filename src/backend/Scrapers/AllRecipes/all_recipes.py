...
class AllRecipesScraper(BaseScraper):
  ...
  @classmethod
  def get_recipe_data(cls, base_url):
    """Fetch recipe IDs and names from the Allrecipes search results."""
    recipes = []
  ...
  def get_all_possible_elements(cls, target) -> []:
    ...
    recipe_data = cls.get_recipe_data(
      f'https://www.allrecipes.com/search?q={target.search_query}'
    )
    new_indexes = set(recipe['id'] for recipe in recipe_data)
    new_possible_elements = new_indexes - old_indexes
    new_targets = [
      recipe for recipe in recipe_data if recipe['id'] in new_possible_elements
    ]
    return [
